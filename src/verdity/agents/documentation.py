...
DOC_PATTERNS: list[tuple[str, str, str, str]] = [
    (
        ...
        r"re:def \w+\(.*\):\s*\n\s*(?!\"\"\"|\'\'\')",
        "info",
        "Function may lack docstring — add docstring for API clarity",
    ),
    ...
    (
        ...
        "breaking change",
        "medium",
        "Potential breaking change noted — verify migration docs",
    ),
]
                    ...
                    if m:
                        ...
                        findings.append(
                            Finding(
                                ...
                                evidence=[
                                    EvidenceItem(tool="docs_analyzer", query=pattern, result=name)
                                ],
                            ...
                            if actual_pattern.lower() in line.lower():
                                findings.append(
                                    Finding(
                                        ...
                                        evidence=[
                                            EvidenceItem(
                                                tool="docs_analyzer", query=pattern, result=name
                                            )
                                        ],
                                        agent_version=self.AGENT_VERSION,

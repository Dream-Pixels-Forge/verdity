...
class TestScanDiffForVulnerabilities:
    ...
    def test_os_system_detected(self, agent):
        ...
        findings = agent._scan_diff_for_vulnerabilities(diff_files)
        assert any("os system" in f.summary.lower() or "command injection" in f.explanation.lower()
                    for f in findings)
    ...
    def test_pickle_load_detected(self, agent):
        ...
        findings = agent._scan_diff_for_vulnerabilities(diff_files)
        assert any("pickle" in f.summary.lower() or "deserialization" in f.summary.lower()
                    for f in findings)
    ...
    def test_weak_hash_detected(self, agent):
        ...
        findings = agent._scan_diff_for_vulnerabilities(diff_files)
        assert any("weak hash" in f.summary.lower() or "md5" in f.summary.lower()
                    for f in findings)

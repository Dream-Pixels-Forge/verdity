...
from verdity.schemas import (
    ConcernType, Finding, QueueEnvelope, RepoRef, ReviewPolicy,
    Severity, SpecialistResponse, TriggerType, VerdityEvent,
)
...
from verdity.verification_gate import (
    CheckResult, GateCheck, GateVerdict, RegressionRunner,
    VerificationGate, VerifierSubagent,
)
...
class TestDocumentationAgentCoverage:
    ...
    async def test_detects_changelog_and_breaking_change(self, services):
        ...
        result = await agent.run(
            ctx=SpecialistContext(
                review_run_id=uuid.uuid4(), repo_owner="acme", repo_name="w",
                base_sha="", head_sha="", diff_files=diff_files, policy=ReviewPolicy(),
            ),
    ...
    async def test_audit_log_records(self, services):
        ...
        run_id = uuid.uuid4()
        diff_files = [{
            "path": "src/x.py", "content": "def foo(): pass",
            "additions": "def foo(): pass\n# TODO: finish\n", "deletions": "",
        }]
        await agent.run(
            ctx=SpecialistContext(
                review_run_id=run_id, repo_owner="acme", repo_name="w",
                base_sha="", head_sha="", diff_files=diff_files, policy=ReviewPolicy(),
            ),
            semantic_index=services["index"],
            token_economics=services["token_economics"],
    ...
    async def test_no_matching_patterns(self, services):
        ...
        result = await agent.run(
            ctx=SpecialistContext(
                review_run_id=uuid.uuid4(), repo_owner="acme", repo_name="w",
                base_sha="", head_sha="",
                diff_files=[{"path": "x.py", "content": "import os", "additions": "import os\n", "deletions": ""}],
                policy=ReviewPolicy(),
            ),
            semantic_index=services["index"],
            token_economics=services["token_economics"],
...
class TestSecurityAgentSemantic:
    ...
    async def test_semantic_search_with_index(self, services):
        ...
        await idx.upsert_chunks([
            CodeChunk(
                chunk_id="c1", repo_id="acme/widgets", file_path="src/auth.py",
                start_line=1, end_line=3, content="def authenticate(password):\n    return verify(password)",
                language="python",
            )
        ])
        diff_files = [{"path": "src/auth.py", "content": "verify(p)", "additions": "verify(p)\n", "deletions": ""}]
        result = await agent.run(
            ctx=SpecialistContext(
                review_run_id=uuid.uuid4(), repo_owner="acme", repo_name="widgets",
                base_sha="", head_sha="", diff_files=diff_files, policy=ReviewPolicy(),
            ),
            semantic_index=idx,
            token_economics=services["token_economics"],
    ...
    async def test_semantic_search_ignores_out_of_diff_files(self, services):
        ...
        await idx.upsert_chunks([
            CodeChunk(
                chunk_id="c1", repo_id="acme/widgets", file_path="src/other.py",
                start_line=1, end_line=1, content="password = secret_value",
                language="python",
            )
        ])
        diff_files = [{"path": "src/different.py", "content": "x", "additions": "x\n", "deletions": ""}]
        result = await agent.run(
            ctx=SpecialistContext(
                review_run_id=uuid.uuid4(), repo_owner="acme", repo_name="widgets",
                base_sha="", head_sha="", diff_files=diff_files, policy=ReviewPolicy(),
...
class TestAggregatorSummary:
    def test_summary_includes_medium_low_info_counts(self):
        ...
        responses = [SpecialistResponse(
            review_run_id=uuid.uuid4(), specialist="sec", status="complete",
            findings=findings,
        )]
    ...
    def test_summary_truncates_after_10_findings(self):
        ...
        findings = [
            Finding(
                ...
                file=f"x{i}.py", line_start=i + 1, line_end=i + 1,
                summary=f"F{i}", explanation="e", confidence=0.8,
                evidence=[], agent_version="v", prompt_hash="h",
            )
            for i in range(12)
        ]
...
class TestApprovalQueueStats:
    ...
    async def test_resolve_and_stats(self):
        ...
        try:
            run_id = uuid.uuid4()
            await store.enqueue(
                run_id=run_id, finding_id=uuid.uuid4(), repo_id="acme/w",
                concern="q", severity="low", file="f.py", line_start=1,
                summary="s", explanation=None, confidence=0.4,
    ...
    async def test_stats_filters_by_repo(self):
        ...
        try:
            for repo_id in ["a/w1", "a/w2"]:
                await store.enqueue(
                    run_id=uuid.uuid4(), finding_id=uuid.uuid4(), repo_id=repo_id,
                    concern="q", severity="low", file="f.py", line_start=1,
                    summary="s", explanation=None, confidence=0.4,
                    route_action="auto_dismiss", route_reason="low",
                )
...
class TestEventQueueCoverage:
    ...
    async def test_nack_exhausts_retries_and_becomes_dead(self):
        ...
        await q.connect()
        try:
            msg_id = await q.publish(QueueEnvelope(
                event=VerdityEvent(
                    delivery_id=str(uuid.uuid4()), trigger_type=TriggerType.PR_OPENED,
...
class TestTokenEconomicsFilters:
    ...
    async def test_get_spend_with_org_filter(self, token_economics):
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="a", model="m",
            input_tokens=100, output_tokens=50, repo_owner="o1",
            repo_name="r1", org="ORG-A",
        )
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="a", model="m",
            input_tokens=200, output_tokens=100, repo_owner="o2",
            repo_name="r2", org="ORG-B",
        )
    ...
    async def test_get_spend_with_since_filter(self, token_economics):
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="a", model="m",
            input_tokens=100, output_tokens=50, repo_owner="o", repo_name="r",
        )
...
class TestBudgetEnforcerExtra:
    ...
    async def test_dashboard_stats(self, token_economics):
        ...
        await token_economics.record_call(
            ...
            repo_owner="acme", repo_name="w", org="acme-org",
        )
...
class TestCodingAgentExtra:
    def test_eval_fix(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.HIGH,
            file="x.py", line_start=1, line_end=1,
            summary="Eval usage detected", explanation="eval(user_input)",
    ...
    def test_exec_fix(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.HIGH,
            file="x.py", line_start=1, line_end=1,
            summary="Exec usage detected", explanation="exec(user_code)",
            confidence=0.85, evidence=[], agent_version="v", prompt_hash="h",
        )
        fix = agent.propose_fix(finding)
        assert fix is not None
    ...
    def test_pickle_fix(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.HIGH,
            file="x.py", line_start=1, line_end=1,
            summary="Unsafe pickle loading", explanation="pickle.load(f)",
            confidence=0.85, evidence=[], agent_version="v", prompt_hash="h",
        )
        fix = agent.propose_fix(finding)
        assert fix is not None
    ...
    def test_hash_fix(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.HIGH,
            file="x.py", line_start=1, line_end=1,
            summary="Weak hash MD5", explanation="hashlib.md5(data)",
            confidence=0.85, evidence=[], agent_version="v", prompt_hash="h",
        )
        fix = agent.propose_fix(finding)
        assert fix is not None
    ...
    def test_no_fix_for_unknown_concern(self):
        agent = CodingAgent()
        finding = Finding(
            concern=ConcernType.DOCUMENTATION, severity=Severity.LOW,
            file="x.py", line_start=1, line_end=1,
            summary="Missing docstring", explanation="e",
...
class TestVerificationGateExtra:
    def test_lint_fail_print_still_present(self):
        ...
        finding = Finding(
            concern=ConcernType.CODE_QUALITY, severity=Severity.MEDIUM,
            file="x.py", line_start=1, line_end=1,
            summary="Bare except", explanation="e",
            confidence=0.7, evidence=[], agent_version="v", prompt_hash="h",
        )
        ...
        fix = ProposedFix(
            ...
            explanation="fix", fix_type="except_specific",
        )
    ...
    def test_compile_fail_mismatched_parens(self):
        ...
        fix = ProposedFix(
            ...
            explanation="bad", fix_type="secret_removal",
        )
        verdict = gate.run_checks(fix, finding)
    ...
    def test_all_checks_pass(self):
        ...
        fix = ProposedFix(
            finding_id=finding.finding_id, file=finding.file,
            original_line=finding.line_start,
            suggested_lines=["import logging", "from verdity.config import settings", "logger = logging.getLogger(__name__)"],
            explanation="good fix", fix_type="secret_removal",
        )
        verdict = gate.run_checks(fix, finding)
...
class TestOrchestratorExtra:
    def test_push_event_policy(self):
        event = VerdityEvent(
            delivery_id=str(uuid.uuid4()), trigger_type=TriggerType.PUSH,
            repo=RepoRef(owner="o", name="r", id=1),
        )
...
class TestSemanticIndexExtra:
    ...
    def test_dev_embedding_deterministic(self):
        ...
        chunks = [CodeChunk(
            chunk_id="c1", repo_id="r", file_path="x.py",
            start_line=1, end_line=2, content="def foo(): pass",
            language="python",
        )]
    ...
    async def test_incremental_upsert_update(self):
        ...
        try:
            await idx.upsert_chunks([
                CodeChunk(
                    chunk_id="c1", repo_id="repo1", file_path="a.py",
                    start_line=1, end_line=2, content="old content",
                    language="python",
                )
            ])
            ...
            await idx.upsert_chunks([
                CodeChunk(
                    chunk_id="c1", repo_id="repo1", file_path="a.py",
                    start_line=1, end_line=2, content="new content",
                    language="python",
                )
            ])
    ...
    async def test_search_by_text_matches_content(self):
        ...
        try:
            await idx.upsert_chunks([
                CodeChunk(
                    chunk_id="c1", repo_id="r", file_path="auth.py",
                    start_line=1, end_line=1, content="def authenticate(password):",
                    language="python",
                )
            ])
...
class TestApprovalQueueNoRepoFilter:
    ...
    async def test_get_pending_no_repo_filter(self):
        ...
        try:
            for i in range(3):
                await store.enqueue(
                    run_id=uuid.uuid4(), finding_id=uuid.uuid4(), repo_id=i,
                    concern="q", severity="low", file="f.py", line_start=1,
                    summary="s", explanation=None, confidence=0.4,
        ...
        try:
            for i in range(2):
                await store.enqueue(
                    ...
                    concern="q", severity="low", file="f.py", line_start=1,
                    summary="s", explanation=None, confidence=0.4,
                    route_action="manual_review", route_reason="low",
                )
...
class TestBudgetEnforcerReturnNone:
    ...
    async def test_warn_no_optional_returns_warn(self, token_economics):
        ...
        status = await enf.check_budget(
            repo_owner="acme", repo_name="w", budget_usd=0.5,
...
class TestCodingAgentNoneReturn:
    def test_security_unknown_issue_returns_none(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.MEDIUM,
            file="x.py", line_start=1, line_end=1,
            summary="Unknown security issue", explanation="e",
            confidence=0.5, evidence=[], agent_version="v", prompt_hash="h",
        )
        assert agent.propose_fix(finding) is None
...
class TestVerificationGateAllFixTypes:
    ...
    def _run_intent_check(self, fix_type, suggested_lines, expected_result):
        ...
        fix = ProposedFix(
            ...
            explanation="test", fix_type=fix_type,
        )
    ...
    def test_unknown_fix_type_skips_intent(self):
        ...
        fix = ProposedFix(
            ...
            explanation="noop", fix_type="unknown_type",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
...
class TestSemanticIndexSearchEdgeCases:
    ...
    async def test_semantic_search_mismatched_dim(self):
        ...
        try:
            await idx.upsert_chunks([
                CodeChunk(chunk_id="c1", repo_id="r", file_path="x.py",
                          start_line=1, end_line=1, content="test", language="py"),
            ])
...
class TestOrchestratorExceptions:
    ...
    async def test_specialist_raises_during_gather(self, services, queue):
        ...
        orch = Orchestrator(
            queue=queue, semantic_index=services["index"],
            token_economics=services["token_economics"], audit_store=services["audit"],
        )
    ...
    async def test_task_exception_during_gather(self, services, queue):
        ...
        async def fast_agent(*args, **kwargs):
            return SpecialistResponse(
                review_run_id=uuid.uuid4(), specialist="security", status="complete",
                findings=[],
            )
        orch = Orchestrator(
            queue=queue, semantic_index=services["index"],
        ...
        try:
            ...
            for i in range(10):
                chunks.append(CodeChunk(
                    chunk_id=f"c{i}", repo_id="r", file_path=f"x{i}.py",
                    start_line=1, end_line=1, content=f"code {i}",
                    language="python",
                ))
            await idx.upsert_chunks(chunks)
            # Query with a vector that matches all
            query_chunks = [CodeChunk(
                chunk_id="q", repo_id="r", file_path="q.py",
                start_line=1, end_line=1, content="code 0",
                language="python",
            )]
...
class TestTokenEconomicsSpecificSignals:
    ...
    async def test_halt_signal(self, token_economics):
        ...
        status = await enf.check_budget("o", "r", budget_usd=0.001, current_specialists=["security"])
    ...
    async def test_normal_signal(self, token_economics):
        from verdity.budget_enforcer import BudgetEnforcer
        enf = BudgetEnforcer(token_economics)
        status = await enf.check_budget("o", "r", budget_usd=100.0, current_specialists=["security"])
...
class TestVerificationGateLintSecret:
    ...
    def test_lint_wildcard_import_fail(self):
        ...
        finding = Finding(
            concern=ConcernType.CODE_QUALITY, severity=Severity.MEDIUM,
            file="x.py", line_start=1, line_end=1,
            summary="Wildcard import", explanation="e",
            confidence=0.7, evidence=[], agent_version="v", prompt_hash="h",
        )
        from verdity.coding_agent import ProposedFix
        fix = ProposedFix(
            ...
            explanation="fix", fix_type="import_specific",
        )
        verdict = gate.run_checks(fix, finding)
        lint_check = next(c for c in verdict.checks if c.name == "lint_pass")
        assert lint_check.result == CheckResult.FAIL
...
class TestGatewayCoverage:
    ...
    async def test_middleware_valueerror_content_length(self, gateway_client):
        ...
        sig = compute_signature(
            __import__("os").environ.get("WEBHOOK_HMAC_SECRET", "test-hmac-secret-key-for-dev-only").encode(),
            body,
        )
        resp = await gateway_client.post(
            "/verdity/webhooks/github",
...
class TestTokenEconomicsThresholds:
    ...
    async def test_warn_threshold_exact(self, token_economics):
        ...
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="sec", model="m",
            input_tokens=400_000, output_tokens=400_000,
            repo_owner="o", repo_name="r",
        )
    ...
    async def test_degrade_threshold_exact(self, token_economics):
        ...
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="sec", model="m",
            input_tokens=300_000, output_tokens=300_000,
            repo_owner="o", repo_name="r",
        )
...
class TestVerificationGateSecretBypass:
    def test_comment_with_secret_passes(self):
        ...
        fix = ProposedFix(
            ...
            explanation="commented secret", fix_type="secret_removal",
        )
        verdict = gate.run_checks(fix, finding)
        secret_check = next(c for c in verdict.checks if c.name == "no_new_secrets")
        assert secret_check.result == CheckResult.PASS
    ...
    def test_env_var_with_secret_passes(self):
        ...
        fix = ProposedFix(
            ...
            explanation="env var", fix_type="secret_removal",
        )
        verdict = gate.run_checks(fix, finding)
        secret_check = next(c for c in verdict.checks if c.name == "no_new_secrets")
        assert secret_check.result == CheckResult.PASS
    ...
    def test_eval_replacement_fail_no_safe_alternative(self):
        ...
        fix = ProposedFix(
            ...
            explanation="unsafe", fix_type="eval_replacement",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
    ...
    def test_pickle_replacement_fail_no_json(self):
        ...
        fix = ProposedFix(
            ...
            explanation="unsafe", fix_type="pickle_replacement",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
    ...
    def test_hash_fix_fail_no_sha256(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.HIGH,
            file="x.py", line_start=1, line_end=1,
            summary="Weak hash", explanation="e",
            confidence=0.85, evidence=[], agent_version="v", prompt_hash="h",
        )
        from verdity.coding_agent import ProposedFix
        fix = ProposedFix(
            ...
            explanation="weak", fix_type="hash_fix",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
    ...
    def test_print_to_logging_fail_no_logger(self):
        ...
        finding = Finding(
            concern=ConcernType.CODE_QUALITY, severity=Severity.LOW,
            file="x.py", line_start=1, line_end=1,
            summary="Print statement", explanation="e",
            confidence=0.5, evidence=[], agent_version="v", prompt_hash="h",
        )
        from verdity.coding_agent import ProposedFix
        fix = ProposedFix(
            ...
            explanation="bad", fix_type="print_to_logging",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
...
class TestGatewayErrorPaths:
    ...
    async def test_normalize_error_returns_400(self, gateway_client, settings):
        ...
        body = json.dumps(payload, separators=(",", ":")).encode()
        sig = compute_signature(settings.webhook_hmac_secret.get_secret_value().encode(), body)
        with patch("verdity.gateway.app.normalize_webhook", side_effect=ValueError("normalization failed")):
            resp = await gateway_client.post(
                "/verdity/webhooks/github",
                content=body,
                headers={
...
class TestOrchestratorGatherExceptions:
    ...
    async def test_task_cancelled_error_handled(self, services, queue):
        ...
        async def cancel_me_agent(run, index, te, audit):
            ...
            return SpecialistResponse(
                review_run_id=run.review_run_id, specialist="sec", status="complete",
                findings=[],
            )
        orch = Orchestrator(
            queue=queue, semantic_index=services["index"],
...
class TestVerificationGateVerifierIntent:
    def test_eval_replacement_pass_with_literal_eval(self):
        ...
        fix = ProposedFix(
            ...
            explanation="safe", fix_type="eval_replacement",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
    ...
    def test_except_specific_fail_no_parens(self):
        ...
        fix = ProposedFix(
            ...
            explanation="still bare", fix_type="except_specific",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
...
class TestGatewayLifespan:
    ...
    async def test_body_too_large_after_read(self, gateway_client, settings):
        ...
        sig = compute_signature(settings.webhook_hmac_secret.get_secret_value().encode(), large_body)
        delivery = str(uuid.uuid4())
    ...
    async def test_orchestrator_gather_cancels_pending(self, services, queue):
        ...
        with patch("asyncio.wait", new=AsyncMock(return_value=({mock_done_task}, {mock_pending_task}))):
            await orch._gather_results(run.review_run_id, run, policy, {"security": mock_pending_task})
    ...
    async def test_orchestrator_gather_skips_already_set(self, services, queue):
        ...
        run.specialist_results["security"] = SpecialistResponse(
            review_run_id=run.review_run_id, specialist="sec",
...
class TestTokenEconomicsExactThresholds:
    ...
    async def test_warn_signal_exact_0_8(self):
        ...
        try:
            ...
            await svc.record_call(
                review_run_id=uuid.uuid4(), agent_name="sec", model="deepseek-chat",
                input_tokens=762_000, output_tokens=762_000,
                repo_owner="o", repo_name="r",
            )
            result = await svc.check_budget_enforcement(repo_owner="o", repo_name="r", budget_usd=0.4)
    ...
    async def test_degrade_optional_signal_exact_0_6(self):
        ...
        try:
            ...
            await svc.record_call(
                review_run_id=uuid.uuid4(), agent_name="sec", model="deepseek-chat",
                input_tokens=715_000, output_tokens=715_000,
                repo_owner="o", repo_name="r",
            )
            result = await svc.check_budget_enforcement(repo_owner="o", repo_name="r", budget_usd=0.5)
...
class TestVerificationGatePrintToLogging:
    def test_print_to_logging_pass(self):
        ...
        fix = ProposedFix(
            ...
            explanation="good", fix_type="print_to_logging",
        )
        verifier = VerifierSubagent()
        verdict = gate.run_checks(fix, finding, verifier=verifier)
        intent_check = next(c for c in verdict.checks if c.name == "matches_intent")
...
class TestRegressionRunner:
    def test_run_regression_affected_scope(self):
        ...
        finding = Finding(
            concern=ConcernType.SECURITY, severity=Severity.HIGH,
            file="x.py", line_start=1, line_end=1,
            summary="test", explanation="e", confidence=0.8,
            evidence=[], agent_version="v", prompt_hash="h",
        )
        from verdity.coding_agent import ProposedFix
        fix = ProposedFix(
            finding_id=finding.finding_id, file=finding.file,
            original_line=finding.line_start,
            suggested_lines=["pass"], explanation="noop", fix_type="secret_removal",
        )
...
class TestSemanticIndexCoverage:
    ...
    async def test_semantic_search_with_similarity(self):
        ...
        try:
            await idx.upsert_chunks([
                CodeChunk(
                    chunk_id="c1", repo_id="r", file_path="auth.py",
                    start_line=1, end_line=2, content="def login(password):",
                    language="python",
                )
            ])
            gen = DevEmbeddingGenerator()
            chunks = [CodeChunk(
                chunk_id="q1", repo_id="r", file_path="q.py",
                start_line=1, end_line=1, content="login",
                language="python",
            )]
    ...
    async def test_delete_chunks_for_file(self):
        ...
        try:
            await idx.upsert_chunks([
                CodeChunk(chunk_id="c1", repo_id="r", file_path="x.py",
                          start_line=1, end_line=1, content="a", language="py"),
                CodeChunk(chunk_id="c2", repo_id="r", file_path="x.py",
                          start_line=2, end_line=2, content="b", language="py"),
            ])
    ...
    async def test_upsert_edges(self):
        ...
        try:
            ...
            edges = [SymbolEdge(
                edge_id="e1", repo_id="r", source_symbol="a",
                target_symbol="b", edge_type="calls",
            )]
...
class TestBudgetEnforcerWarnPath:
    ...
    async def test_degrade_preemptive_drop(self, token_economics):
        ...
        status = await enf.check_budget(
            repo_owner="acme", repo_name="w", budget_usd=0.3,
...
class TestCodingAgentQualityFixes:
    def test_wildcard_import_fix(self):
        ...
        finding = Finding(
            concern=ConcernType.CODE_QUALITY, severity=Severity.LOW,
            file="x.py", line_start=1, line_end=1,
            summary="Wildcard import detected", explanation="e",
            confidence=0.5, evidence=[], agent_version="v", prompt_hash="h",
        )
        fix = agent.propose_fix(finding)
        assert fix is not None
    ...
    def test_quality_unknown_issue_returns_none(self):
        ...
        finding = Finding(
            concern=ConcernType.CODE_QUALITY, severity=Severity.LOW,
            file="x.py", line_start=1, line_end=1,
            summary="Some unknown issue", explanation="e",
            confidence=0.3, evidence=[], agent_version="v", prompt_hash="h",
        )
        assert agent.propose_fix(finding) is None
...
class TestTokenEconomicsExtra:
    ...
    async def test_get_spend_with_repo_filter(self, token_economics):
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="a", model="m",
            input_tokens=100, output_tokens=50, repo_owner="o", repo_name="r1",
        )
        await token_economics.record_call(
            review_run_id=uuid.uuid4(), agent_name="a", model="m",
            input_tokens=200, output_tokens=100, repo_owner="o", repo_name="r2",
        )
